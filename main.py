...
def get_most_recent_10_k(cik: int) -> tuple[str, str]:
    ...
    if not ten_k_indices:
        raise ValueError("Did not find 10-K")

    most_recent_index = max(ten_k_indices, key=lambda i: filings["filingDate"][i])
...
def get_10_k_file_url(cik: int, accession_number: str) -> str:
    file = sec_request(
    ...
    for line in file:
        ...
        raise Exception("No file name found in 10-K accession file")

    print(f"10-K file name: {file_name}")
...
def save_file_to_pdf(url: str, output_file_name: str) -> None:
    ...
    print(f"Downloaded 10-K: {len(file.content)} bytes")
...
def get_cik_for_company(identifier: str):
    ...
    for item in data:
        ...
        if identifier == name or identifier == ticker:
            ...
            return cik, item[name_index]

    raise Exception(f"No CIK found for {identifier}")


def create_most_recent_10_k_for_company(identifier: str, output_file_name: str = None):
    cik, company_name = get_cik_for_company(identifier)
    accession_number, filing_date = get_most_recent_10_k(cik)
    file_url = get_10_k_file_url(cik, accession_number)
    file_name = output_file_name or f"{format_cik(cik)}.pdf"
...
if __name__ == "__main__":
    ...
    try:
        output = create_most_recent_10_k_for_company(identifier)
